...
if __name__ == "__main__":
    ...
    for run in range(runs):
        ...
        pool = MaxPool(2)
        full = Fully_Connected(441, 8, adam_lr=0.1)

        epoch_train_losses, epoch_test_losses = train_network(X_train, y_train, conv, pool, full, X_test, y_test, lr=0.1, epochs=20)
        
        with open(f"conv_params{run}.pkl", "wb") as f:
            pickle.dump({'filters': conv.filters, 'biases': conv.biases}, f)

        with open(f"pool_params{run}.pkl", "wb") as f:
            pickle.dump({'pool_size': pool.pool_size}, f)

        with open(f"full_params{run}.pkl", "wb") as f:
            pickle.dump({'weights': full.weights.numpy(), 'biases': full.biases.numpy()}, f)
